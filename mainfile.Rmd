...
author: "El Brahmi, Lazic, Schwaiger, Zaric"
---
...
df <- read.csv("Datensatz2.csv", sep = ",")
```
...
Das Histogramm der Temperatur zeigt eine leicht asymmetrische Verteilung mit\
...
tsb <- df %>%
  ...
  fill_gaps() %>%
  mutate(meantemp = zoo::na.approx(meantemp, na.rm = FALSE))
tsb %>%
...
Das ist eigentlich unglaublich langweilg anzusehen, da es sich um saisonale, also sich wiederholdende Daten handelt.
...
Andere Darstellungsformen sind nicht viel besser, da die Darstellung auf die Saisonalität keinen Einfluss hat.

**Fazit**: Wenn man mit Regressionen arbeitet, sollte man die Daten zuvor "Desaisonalisieren".

Wir schreiten zum 2. Versuch: Glättungen pro Jahr
